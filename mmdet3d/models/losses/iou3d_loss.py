...
def diou_3d_loss(pred, target, eps=1e-6):
    ...
    cnd = torch.sum(torch.mean((corners_from_bboxes3d(pred) - corners_from_bboxes3d(target)) ** 2, dim=1), dim=-1)
    did = torch.max(torch.sum(pred[:, 3:6] ** 2, dim=-1), torch.sum(target[:, 3:6] ** 2, dim=-1))
