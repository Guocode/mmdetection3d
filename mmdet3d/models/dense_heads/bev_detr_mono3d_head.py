...
class BEVDETRMono3DHead(BaseMono3DDenseHead):
    ...
    def _init_layers(self):
        ...
        bev_ltcenterxz_priors = self.get_bev_ltcenterxz_priors(1, self.bev_prior_size, self.zrange,
                                                               710 / 640, torch.float32, 'cpu', flatten=False)
    ...
    def _init_predictor(self):
        ...
        self.conv_reg_ffn = nn.Sequential(
            nn.Linear(
                self.in_channels * (2 ** self.stacked_convs),
                self.in_channels * (2 ** (self.stacked_convs - 1))
            ),
            nn.ReLU(inplace=True),
            nn.Linear(
                self.in_channels * (2 ** (self.stacked_convs - 1)),
                self.in_channels * (2 ** (self.stacked_convs - 2))
            ),
            nn.ReLU(inplace=True),
            nn.Linear(
                self.in_channels * (2 ** (self.stacked_convs - 2)),
    ...
    def init_weights(self):
        ...
        normal_init(self.conv_reg_ffn, std=0.01, bias=bias_init_with_prob(0.01))
    ...
    def forward(self, feats):
        ...
        bev_feats = F.adaptive_avg_pool2d(bev_feats, self.bev_ltcenterxz_priors.shape[2:])
        bev_feats = bev_feats + self.bev_enc(self.bev_ltcenterxz_priors)
        cls_reg_feat = F.adaptive_avg_pool2d(bev_feats, (10, 10))
        cls_reg_feat = self.clsreg_convs(cls_reg_feat)
        # cls_reg_feat = F.adaptive_avg_pool2d(cls_reg_feat, 1)
        # cls_reg_feat = F.adaptive_avg_pool2d(cls_reg_feat, (10,10))

        cls_reg_out = self.conv_reg_ffn(cls_reg_feat.view(b, -1, self.num_query).permute(0, 2, 1))
        ...
        reg_xyz_out = torch.tanh(reg_out[..., 0:3])  # (-1,1)
    ...
    def get_targets_single_img(self, cls_out, reg_out, decode_bboxes3d, gt_bboxes_3d, gt_labels_3d, img_meta):
        ...
        gt_bboxes_3d_ass_xyz = (gt_bboxes_3d[:, :3] - self.xyzrange[:, 0][None]) / (
                self.xyzrange[:, 1] - self.xyzrange[:, 0]) * 2 - 1
        gt_bboxes_3d_target_xyz = gt_bboxes_3d_ass_xyz
        gt_bboxes_3d_ass_dim = gt_bboxes_3d[:, 3:6] / self.canon_box_sizes[gt_labels_3d] / 60
        gt_bboxes_3d_target_dim = (gt_bboxes_3d[:, 3:6] / self.canon_box_sizes[gt_labels_3d]).clamp(0.5, 2.)
        gt_bboxes_3d_target = torch.cat([gt_bboxes_3d_target_xyz, gt_bboxes_3d_target_dim, gt_bboxes_3d[:, 6:]], dim=-1)
        gt_bboxes_3d_ass = torch.cat([gt_bboxes_3d_ass_xyz, gt_bboxes_3d_ass_dim, gt_bboxes_3d[:, 6:]], dim=-1)
        decode_dim_ass = decode_bboxes3d[:, 3:6] / 60
        decode_bboxes3d_ass = torch.cat([decode_bboxes3d[:, :3], decode_dim_ass, decode_bboxes3d[:, 6:]], dim=-1)
    ...
    def get_bev_ltcenterxz_priors(
            self, batch_size, bev_size, zrange, ux_div_fdx, dtype, device, flatten=True,
    ):
