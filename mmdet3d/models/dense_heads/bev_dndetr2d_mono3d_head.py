...
class BEVDNDETRMono3DHead(BaseMono3DDenseHead):
    def __init__(self,
        ...
        self.in_h = 2
        self.in_w = 2
        self.xyz_prior = nn.Parameter(torch.normal(0, 0.1, (1, 12 * 40, 3)))
        if self.train_cfg:
    ...
    def _init_layers(self):
        ...
        self.ffn = nn.Sequential(
            nn.Dropout(0.1),
            nn.Conv2d(self.feat_channels, self.cls_out_channels + self.reg_out_channels + 4, 1),
        )
        self.init_weights()
    ...
    def forward(self, feats, content_query=None, positional_query=None):
        ...
        cls_out, reg_out, reg2d_out = cls_reg_out[..., :self.cls_out_channels], cls_reg_out[...,
                                                                                self.cls_out_channels:-4], cls_reg_out[
                                                                                                           ..., -4:]
        ...
        reg_cxcywh_out = reg2d_out
        regs_out = torch.cat([reg_xyz_out, reg_dim_out, reg_roty_out,reg_cxcywh_out], dim=-1)
