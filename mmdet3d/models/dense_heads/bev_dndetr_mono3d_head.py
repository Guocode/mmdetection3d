...
class BEVDNDETRMono3DHead(BaseMono3DDenseHead):
    ...
    def forward(self, feats):
        ...
        reg_dim_out = torch.tanh(reg_out[..., 3:6]) + 1  # (0,2)
    ...
    def get_targets_single_img(self, cls_out, reg_out, decode_bboxes3d, gt_bboxes_3d, gt_labels_3d, img_meta):
        ...
        gt_bboxes_3d_ass_xyz = torch.cat([gt_bboxes_3d[:, :1],gt_bboxes_3d[:, 1:2]-gt_bboxes_3d[:, 1:2],gt_bboxes_3d[:, 2:3]],dim=-1)
        gt_bboxes_3d_ass_dim = gt_bboxes_3d[:, 3:6]/gt_bboxes_3d[:, 3:6]# / self.canon_box_sizes[gt_labels_3d] / 30
        ...
        decode_ass_xyz = torch.cat([decode_bboxes3d[:, :1],decode_bboxes3d[:, 1:2]-decode_bboxes3d[:, 1:2],decode_bboxes3d[:, 2:3]],dim=-1)
        decode_ass_dim = decode_bboxes3d[:, 3:6]/decode_bboxes3d[:, 3:6]# / 30  # / decode_bboxes3d[:, 3:6]
    ...
    def get_dntargets_single_img(self, cls_out, reg_out, decode_bboxes3d, gt_bboxes_3d, gt_labels_3d, img_meta):
        ...
        decode_ass_xyz = torch.cat([decode_bboxes3d[:, :1],decode_bboxes3d[:, 1:2]-decode_bboxes3d[:, 1:2],decode_bboxes3d[:, 2:3]],dim=-1)
        decode_ass_dim = decode_bboxes3d[:, 3:6]/decode_bboxes3d[:, 3:6]   # / decode_bboxes3d[:, 3:6]
        decode_ass_roty = decode_bboxes3d[:, 6:] - decode_bboxes3d[:, 6:]
        decode_bboxes3d_ass = torch.cat([decode_ass_xyz, decode_ass_dim, decode_ass_roty], dim=-1)
    ...
    def get_bev_priors(
        ...
        y = y.view(h, w, 1)
        z = z.view(h, w, 1)
        dim = torch.zeros((h, w, 3), dtype=dtype, device=device)
    ...
    def get_loss_from_target(self, cls_out, reg_out, batch_target):
        batch_labels_targets, batch_bboxes3d_targets, batch_pos_inds, batch_labels_dntargets, batch_bboxes3d_dntargets,batch_pos_dninds = batch_target
        ...
        reg_dnout = reg_out
        dn_reg_weight = (1-F.softmax(cls_out,dim=-1).max(-1)[0])*batch_labels_weight
