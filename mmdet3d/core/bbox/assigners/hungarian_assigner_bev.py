...
class HungarianAssignerBEV(BaseAssigner):
    ...
    def assign(self,
        ...
        num_gts, num_bboxes = gt_bboxes.size(0), xz_pred.size(0)

        # 1. assign -1 by default
        assigned_gt_inds = xz_pred.new_full((num_bboxes,),
                                              -1,
                                              dtype=torch.long)
        assigned_labels = xz_pred.new_full((num_bboxes,),
                                             -1,
                                             dtype=torch.long)
        ...
        l1_cost  = torch.cdist(bev_prior, gt_bboxes[:,:], p=1)
        # offset_cost = (xz_pred[:,:]-bev_prior).abs().sum(-1,keepdims=True)
        cost = l1_cost#+offset_cost
        ...
        assigned_num = 0
        assigned_cyc = 0
        max_overlaps = torch.zeros(xz_pred.size(0), dtype=torch.float32, device=xz_pred.device)
        if linear_sum_assignment is None:
            raise ImportError('Please run "pip install scipy" '
                              'to install scipy first.')
        # cycled assign until all preds are assigned
        while assigned_num < xz_pred.size(0)/10 and assigned_cyc<1:
            matched_row_inds, matched_col_inds = linear_sum_assignment(cost_)
            matched_row_inds = torch.from_numpy(matched_row_inds).to(
                xz_pred.device)
            matched_col_inds = torch.from_numpy(matched_col_inds).to(
                xz_pred.device)
            valid_assign = min(xz_pred.size(0)-assigned_num,gt_bboxes.size(0))
